                ...
                {toasts.map((t) => {
                    ...
                    return (
                        <div
                            key={t.id}
                            className={cn(
                                "flex items-start gap-3 p-4 rounded-lg border bg-black text-white border-neutral-800 shadow-lg transition-all duration-300 pointer-events-auto",
                                t.removing ? "translate-x-full opacity-0" : "translate-x-0 opacity-100"
                            )}
                        >
                            {isSuccess && (
                                <svg className="h-5 w-5 text-neutral-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
